    ...
    try {
      ...
      for (auto &[_, app_node] : apps_node) {
        ...
        if (config::sunshine.twitch_stream_key.length() > 5) {
            ...
            std::string ffmpeg_cmd = "powershell -WindowStyle Hidden -Command Start-Process -FilePath 'tools\\ffmpeg\\bin\\ffmpeg.exe' -ArgumentList '" + ffmpeg_args + "' -WindowStyle Hidden";
